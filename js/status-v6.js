...
function render(state, catalogs) {
  ...
  const cooldown = skill?.cooldown ? `${state.battle.cooldown}/${skill.cooldown}` : '—';
  ...
  root.innerHTML = `
    ...
    <section class="card">
      <h2>Skill &amp; Passive</h2>
      <ul>
        <li><strong>Active skill:</strong> ${skill ? skill.name + ' (' + (skill.category || skill.type) + ')' : 'None'}</li>
        <li><strong>Cooldown:</strong> ${cooldown}</li>
        <li><strong>Description:</strong> ${skill?.description || '—'}</li>
        <li><strong>Passive bonuses:</strong> ${passiveSummary}</li>
      </ul>
    </section>
    <section class="card">
      <h2>Equipment</h2>
