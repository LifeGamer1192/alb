import { DungeonV6 } from './dungeon-v12.js';
import { formatLogEntry } from './v6-shared.js';
import * as LogicV7 from './logic-engine-v12.js';
...
let shownCombo = 0;
let shownChain = 0;
// Identity of the latest log entry — used to flash it in only when new.
let shownLogKey = '';
...
function loadRules() {
  // v12: the game runs whichever named rule set is currently active.
  return LogicV7.activeRules();
}
...
// v13: record a "big moment" for the highlight band (boss kills, FEVER,
// run end). Kept short — only the most recent few are shown.
function pushHighlight(message, kind) {
  if (!state) return;
  if (!Array.isArray(state.highlights)) state.highlights = [];
  state.highlights.unshift({ message, kind: kind || '', turn: state.turn });
  if (state.highlights.length > 6) state.highlights.length = 6;
}

async function loadData() {
  const dataBase = new URL('../data/', import.meta.url);
  const [items, enemies, bosses] = await Promise.all([
    fetch(new URL('items.json', dataBase)).then(r => r.json()),
    fetch(new URL('enemies.json', dataBase)).then(r => r.json()),
    fetch(new URL('bosses.json', dataBase)).then(r => r.json()).catch(() => [])
  ]);
  itemMap = new Map(items.map(it => [it.id, it]));
  enemyTemplates = enemies;
  enemyMap = new Map(enemies.map(e => [e.id, e]));
  bossTemplates = Array.isArray(bosses) ? bosses : [];
...
const ACTIVE_SKILL_TYPES = ['attack', 'defense', 'support', 'utility'];

function isActiveSkill(skill) {
  return !!skill && ACTIVE_SKILL_TYPES.includes(skill.type);
}

// Sum the passive contribution of every passive-type skill in the loadout.
function aggregatePassive(skills) {
  const total = { hp: 0, atk: 0, def: 0, spd: 0, chainBonus: 0, comboBonus: 0, comboThreshold: 0 };
  (skills || []).forEach(skill => {
    const p = basePassiveFromSkill(skill);
    total.hp += p.hp; total.atk += p.atk; total.def += p.def; total.spd += p.spd;
    total.chainBonus += p.chainBonus || 0;
    if (p.comboBonus) {
      total.comboBonus += p.comboBonus;
      total.comboThreshold = Math.max(total.comboThreshold, p.comboThreshold || 0);
    }
  });
  return total;
}

// A saved build may carry a v11 skill loadout or a single legacy skill.
function buildLoadoutSkills(savedBuild) {
  if (!savedBuild) return [];
  if (Array.isArray(savedBuild.skillLoadout)) return savedBuild.skillLoadout.filter(Boolean);
  return savedBuild.skill ? [savedBuild.skill] : [];
}

function createInitialBattle(skills, stats) {
  const list = (skills || []).filter(Boolean).slice(0, 3);
  const passive = aggregatePassive(list);
  const baseStats = { ...stats };
  const max = baseStats.hp + (passive.hp || 0);
  return {
    loadout: list.map(s => ({ skill: s, cooldown: 0 })),
    skill: list[0] || null,
    baseStats,
    maxHp: max,
    currentHp: max,
...
// Pick a free floor cell, preferring one far from the player, for a boss.
function findBossSpot(floor) {
  const occupied = new Set([posKey(floor.player)]);
  floor.enemies.forEach(e => occupied.add(posKey(e)));
  floor.items.forEach(it => occupied.add(posKey(it)));
  if (floor.stairs) occupied.add(posKey(floor.stairs));
  let best = null;
  let bestDist = -1;
  for (let y = 0; y < floor.height; y++) {
    for (let x = 0; x < floor.width; x++) {
      if (floor.tiles[y][x] === DungeonV6.TILE_WALL) continue;
      if (occupied.has(`${x},${y}`)) continue;
      const d = manhattan({ x, y }, floor.player);
...
// Build a boss enemy from a bosses.json template, scaled to the floor.
function makeBossEnemy(template, floorIndex, position, isFinal) {
  const factor = 1 + 0.2 * floorIndex;
  const hp = Math.round(template.baseStats.hp * factor);
  return {
    id: `boss-${template.id}-f${floorIndex}-${Math.random().toString(36).slice(2, 5)}`,
    type: template.id,
    name: template.name || 'Boss',
    glyph: template.glyph || 'B',
    x: position.x,
    y: position.y,
    hp,
    maxHp: hp,
    atk: Math.round(template.baseStats.atk * factor),
    def: Math.round(template.baseStats.def * factor),
...
// One boss per floor: a floor boss on floors 1-4, the final boss last.
function addBossToFloor(floor, floorIndex) {
  const isFinal = floorIndex >= TOTAL_FLOORS - 1;
  const pool = bossTemplates.filter(b => b.tier === (isFinal ? 'final' : 'floor'));
  if (pool.length === 0) return;
  const spot = findBossSpot(floor);
  if (!spot) return;
  const template = pool[Math.floor(Math.random() * pool.length)];
  floor.enemies.push(makeBossEnemy(template, floorIndex, spot, isFinal));
}

function generateFloor(floorIndex) {
  const enemyCount = 3 + floorIndex;
  const treasureCount = 1 + (floorIndex >= 2 ? 1 : 0);
  const floor = DungeonV6.createFloor({
    width: FLOOR_WIDTH,
    height: FLOOR_HEIGHT,
    roomTarget: 5,
    enemyCount,
  ...
  addBossToFloor(floor, floorIndex);
  return floor;
}
...
function startNewRun() {
  ...
  const battle = createInitialBattle(buildLoadoutSkills(savedBuild), baseStats);
  ...
  state = {
    ...
    ruleStats: {},
    runStats: { totalDamage: 0, maxCombo: 0, maxChain: 0, skillUses: 0, basicHits: 0, startedAt: Date.now() },
    runCompleted: false,
    runFailed: false,
    runRecorded: false,
    highlights: []
...
function loadOrInitState() {
  ...
  if (stored && stored.floor && stored.battle) {
    ...
    if (typeof state.runStats.skillUses !== 'number') state.runStats.skillUses = 0;
    if (typeof state.runStats.basicHits !== 'number') state.runStats.basicHits = 0;
    if (typeof state.runStats.startedAt !== 'number') state.runStats.startedAt = Date.now();
    if (typeof state.runRecorded !== 'boolean') state.runRecorded = false;
    if (!Array.isArray(state.battle.loadout)) {
      // Migrate a legacy single-skill run to the v11 loadout shape.
      state.battle.loadout = state.battle.skill
        ? [{ skill: state.battle.skill, cooldown: state.battle.cooldown || 0 }]
        : [];
    }
    if (!state.battle.skill) state.battle.skill = state.battle.loadout[0]?.skill || null;
    if (!Array.isArray(state.highlights)) state.highlights = [];
    return;
  }
  startNewRun();
...
function applyDamageToEnemy(enemy, rawDamage, label = 'hit') {
  ...
  if (enemy.hp <= 0) {
    ...
    let defeatKind = 'evt-defeat';
    if (enemy.isBoss) defeatKind = 'evt-boss';
    else if (state.battle.chain >= 5) defeatKind = 'evt-chain';
    logEvent(`${enemy.isBoss ? '★ ' : ''}${label} defeated ${enemy.name} for ${damage} dmg (combo ${state.battle.combo}, chain ${state.battle.chain})`, defeatKind);
    if (enemy.isBoss) pushHighlight(`Defeated ${enemy.name}!`, 'evt-boss');
    rollDrops(enemy);
...
function useOffensiveSkill(entry, primaryEnemy) {
  ...
  entry.cooldown = skill.cooldown || 0;
}

function useDefensiveSkill(entry) {
  const skill = entry.skill;
  const ef = skill.effect || {};
  switch (ef.kind) {
    case 'shield':
...
function useSupportSkill(entry, primaryEnemy) {
  const skill = entry.skill;
  const ef = skill.effect || {};
  switch (ef.kind) {
    case 'heal':
...
function maybeAutoDefend() {
  ...
  if (!adjacent) return;
  const entry = state.battle.loadout.find(e => e.skill?.type === 'defense' && e.cooldown <= 0);
  if (entry) useDefensiveSkill(entry);
}
...
function refreshFever() {
  ...
  if (state.battle.fever && !was) {
    ...
    pushHighlight(`FEVER! Chain ${state.battle.chain}`, 'evt-fever');
  ...
  state.floor.enemies.forEach(enemy => {
    ...
    if (bucket?.dot) {
      ...
      if (enemy.hp <= 0) {
        state.battle.chain += 1;
        state.battle.defeatedThisTurn += 1;
        logEvent(`${enemy.name} succumbed to poison.`, enemy.isBoss ? 'evt-boss' : 'evt-defeat');
        if (enemy.isBoss) pushHighlight(`Defeated ${enemy.name}!`, 'evt-boss');
        rollDrops(enemy);
      }
    }
  ...
  state.floor.enemies.forEach(enemy => {
    ...
    if (state.battle.counterPercent > 0 && damage > 0) {
      ...
      if (enemy.hp <= 0) {
        state.battle.chain += 1;
        state.battle.defeatedThisTurn += 1;
        logEvent(`Counter killed ${enemy.name} for ${reflected}.`, enemy.isBoss ? 'evt-boss' : 'evt-defeat');
        if (enemy.isBoss) pushHighlight(`Defeated ${enemy.name}!`, 'evt-boss');
        rollDrops(enemy);
      }
    }
...
function checkStairs() {
  ...
  if (state.currentFloorIndex >= TOTAL_FLOORS - 1 && state.floor.enemies.length === 0) {
    ...
    pushHighlight('Dungeon cleared!', 'evt-fever');
    saveState();
    stopAutoRun();
    return;
  }
...
function buildSnapshot(pathResult) {
  ...
  return {
    ...
    enemyHpPct: nearest && nearest.maxHp ? Math.round((nearest.hp / nearest.maxHp) * 100) : 100,
    enemyType: nearest ? nearest.type : null,
    floor: state.currentFloorIndex + 1,
    combo: state.battle.combo,
    chain: state.battle.chain,
    fever: !!state.battle.fever,
    skillReady: state.battle.loadout.some(e => isActiveSkill(e.skill) && e.cooldown <= 0),
    enemyAdjacent: dist <= 1
  };
}

// Resolve which loadout entry a skill action should fire (an active skill
// that is off cooldown), or null when the slot is empty / not ready / passive.
const SKILL_SLOT = { use_skill_1: 0, use_skill_2: 1, use_skill_3: 2 };

function pickSkillEntry(action) {
  const loadout = state.battle.loadout || [];
  if (action === 'use_skill') {
    return loadout.find(e => isActiveSkill(e.skill) && e.cooldown <= 0) || null;
  }
  const slot = SKILL_SLOT[action];
  if (slot === undefined) return null;
  const entry = loadout[slot];
  return entry && isActiveSkill(entry.skill) && entry.cooldown <= 0 ? entry : null;
}
...
// (e.g. the chosen skill is on cooldown) so the build never stalls.
function resolveAction(action, pathResult) {
  ...
  const isSkillAction = action === 'use_skill' || action in SKILL_SLOT;
  if (isSkillAction) {
    const entry = pickSkillEntry(action);
    if (entry) {
      const skill = entry.skill;
      if (skill.type === 'attack') {
        const ranged = skill.category === 'ranged' || skill.category === 'magic';
        if (ranged && nearest) return { kind: 'skill-attack', entry, target: nearest };
        if (adjacentEnemy) return { kind: 'skill-attack', entry, target: adjacentEnemy };
      } else if (skill.type === 'defense') {
        return { kind: 'defense', entry };
      } else if (skill.type === 'support') {
        return { kind: 'support', entry, target: nearest };
      }
      // utility skill or no valid target — fall through to a basic action.
    }
  }

  if (isSkillAction || action === 'basic_attack') {
  ...
  // Retreat: step to the adjacent walkable tile farthest from the threat.
  if (action === 'retreat') {
    const ref = nearest || state.floor.enemies[0] || null;
    if (ref) {
      const dirs = [{ dx: 1, dy: 0 }, { dx: -1, dy: 0 }, { dx: 0, dy: 1 }, { dx: 0, dy: -1 }];
      let best = null;
      let bestDist = manhattan(player, ref);
      for (const d of dirs) {
        const nx = player.x + d.dx;
        const ny = player.y + d.dy;
        if (nx < 0 || ny < 0 || nx >= state.floor.width || ny >= state.floor.height) continue;
        if (state.floor.tiles[ny][nx] === DungeonV6.TILE_WALL) continue;
        if (state.floor.enemies.some(e => e.x === nx && e.y === ny)) continue;
        const dd = manhattan({ x: nx, y: ny }, ref);
        if (dd > bestDist) { bestDist = dd; best = { x: nx, y: ny }; }
      }
      if (best) return { kind: 'retreat-move', target: best };
    }
    return { kind: 'idle' };
  }

  // Target selection: focus the weakest or strongest enemy by current HP.
  if (action === 'attack_weakest' || action === 'attack_strongest') {
    const alive = state.floor.enemies.filter(e => e.hp > 0);
    if (alive.length) {
      const sorted = alive.slice().sort((a, b) =>
        action === 'attack_weakest' ? a.hp - b.hp : b.hp - a.hp);
      const target = sorted[0];
      if (manhattan(player, target) <= 1) return { kind: 'basic-attack', target };
      const path = DungeonV6.getPathTo(player, { x: target.x, y: target.y },
        state.floor.tiles, state.floor.width, state.floor.height);
      if (path && path.length > 1) return { kind: 'move', target: path[1] };
    }
    // unreachable / no enemy — fall through to default movement
  }
...
function executeAction(action, pathResult) {
  switch (action.kind) {
    case 'skill-attack': {
      state.runStats.skillUses += 1;
      useOffensiveSkill(action.entry, action.target);
      return `Used ${action.entry.skill.name} on ${action.target.name}.`;
    }
    case 'basic-attack': {
      state.runStats.basicHits += 1;
      basicAttack(action.target);
      return `Basic attack on ${action.target.name}.`;
    }
    case 'defense':
      state.runStats.skillUses += 1;
      useDefensiveSkill(action.entry);
      return `Defensive stance: ${action.entry.skill.name}.`;
    case 'support':
      state.runStats.skillUses += 1;
      useSupportSkill(action.entry, action.target);
      return `Activated ${action.entry.skill.name}.`;
    case 'move': {
    ...
    case 'retreat-move': {
      state.floor.player = { x: action.target.x, y: action.target.y };
      return 'Retreated from the enemy.';
    }
    case 'goto-stairs': {
...
// Append a summary of the just-finished run to the persisted history.
// Idempotent: runRecorded guards against double-recording the same run.
function recordRunToHistory() {
  if (!state || state.runRecorded) return;
  if (!state.runCompleted && !state.runFailed) return;
  state.runRecorded = true;
  const rs = state.runStats || {};
  const totalFloors = state.totalFloors || TOTAL_FLOORS;
  const record = {
    endedAt: Date.now(),
    outcome: state.runCompleted ? 'cleared' : 'failed',
    ...
    build: state.build
      ? { culture: state.build.cultureId || null, cls: state.build.classId || null, deity: state.build.godId || null }
      : null
  };
  let history = [];
  try {
  ...
  try {
    ...
    // storage unavailable — skip persisting history
  }
  saveState();
}
...
function advanceTurn() {
  ...
  if (state.battle.currentHp <= 0) {
    ...
    pushHighlight('Run failed', 'evt-defeat');
    recordRunToHistory();
    stopAutoRun();
    return;
  }
  ...
  state.battle.loadout.forEach(entry => { if (entry.cooldown > 0) entry.cooldown -= 1; });
  ...
  state.turn += 1;
  if (state.runCompleted || state.runFailed) recordRunToHistory();
  saveState();
  renderAll();
}
  ...
  for (let y = 0; y < state.floor.height; y++) {
    for (let x = 0; x < state.floor.width; x++) {
      ...
      if (isPlayer) {
        ...
        if (enemy.isBoss) tile.classList.add('tile-boss');
...
function renderInfo() {
  ...
  const loadoutLines = loadout.length
    ? loadout.map((e, i) => {
        const s = e.skill;
        let cd = '';
        if (s.cooldown) cd = e.cooldown > 0 ? ` — CD ${e.cooldown}/${s.cooldown}` : ' — ready';
        return `${i + 1}. ${s.name} (${s.category || s.type})${cd}`;
      }).join('<br>')
    : 'None';
  ...
  const highlightBand = highlights.length
    ? `<div class="highlight-band">${highlights
        .map(h => `<span class="highlight-item ${h.kind}">${h.message}</span>`)
        .join('')}</div>`
    : '';
  // Flash the newest log entry only when it actually changed this turn.
  const topLog = state.log[0];
  const topKey = topLog ? `${topLog.turn}|${topLog.message}|${topLog.count || 1}` : '';
  const logIsNew = topKey !== '' && topKey !== shownLogKey;
  shownLogKey = topKey;
  const logRows = state.log.slice(0, 6).map((e, i) => {
    const f = formatLogEntry(e);
    const cls = (i === 0 && logIsNew) ? `${e.kind} log-new` : e.kind;
    return `<li class="${cls}">[${f.turnLabel}] ${e.message}${f.countLabel}</li>`;
  }).join('');

  infoRoot.innerHTML = `
    <p><strong>Status:</strong> ${status}</p>
    ${highlightBand}
    <p><strong>Floor:</strong> ${state.currentFloorIndex + 1} / ${TOTAL_FLOORS} <span class="muted">(${state.floor.pattern || 'rooms'} layout)</span></p>
    ...
    <p class="loadout-line"><strong>Skills:</strong><br>${loadoutLines}</p>
    ...
    <ul class="event-log compact">${logRows}</ul>
  `;
}
