...
const AUTO_SPEED_MS = { 1: 800, 2: 400, 4: 200 };
...
function refreshFever() {
  ...
  if (state.battle.fever && !was) logEvent(`FEVER! Chain ${state.battle.chain} — damage surges!`, 'evt-fever');
  else if (!state.battle.fever && was) logEvent('FEVER ended.', '');
}
...
function tickEnemyAi() {
  state.floor.enemies.forEach(enemy => {
    ...
    if (enemy.aiHint === 'regen' && enemy.regen) {
      const heal = enemy.regen;
      enemy.hp = Math.min(enemy.maxHp, enemy.hp + heal);
      if (heal > 0) logEvent(`${enemy.name} regenerated ${heal} HP.`, '');
    }
  });
}
...
function pickAction(pathResult) {
  ...
  const hpRatio = state.battle.currentHp / state.battle.maxHp;
  ...
  if (skill && cdReady) {
    ...
    if (skill.type === 'support' && skill.effect?.kind === 'buff' && (state.battle.combo > 0 || adjacentEnemy)) {
      return { kind: 'support', target: nearest };
    }
    if (skill.type === 'support' && skill.effect?.kind === 'feverPrime' && state.battle.chain >= 3) {
      return { kind: 'support', target: nearest };
    }
...
function renderInfo() {
  ...
  infoRoot.innerHTML = `
    ...
    <p class="combo-line ${comboClass}"><strong>Combo:</strong> ${state.battle.combo}</p>
    <p class="chain-line ${chainClass}"><strong>Chain:</strong> ${state.battle.chain} (FEVER at ${feverThreshold()})</p>
    ${feverLine}
